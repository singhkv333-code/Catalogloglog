...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO users (email, password, otp, otp_expires, is_verified) VALUES ($1, $2, $3, $4, false)`,
...
app.post('/verify-otp', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, otp, otp_expires, is_verified FROM users WHERE email = $1',
      [email]
    )
...
app.post('/login', async (req, res) => {
    ...
    try {
      ...
      if (devUser.rows.length === 0) {
        ...
        devUser = await pool.query(
          `INSERT INTO users (email, password, username, is_verified) VALUES ($1, $2, $3, true) RETURNING id, username`,
  ...
  try {
    const result = await pool.query(
      'SELECT id, email, password, is_verified, username FROM users WHERE email = $1',
      [email]
    )
...
app.post('/set-username', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!result.rows[0].is_verified) return res.status(403).json({ message: 'User not verified' })
...
app.get('/api/users/:id/public', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, username, bio FROM users WHERE id = $1 AND is_verified = true',
      [id]
    )
    if (result.rows.length === 0) return res.status(404).json({ message: 'User not found' })
...
app.put('/api/profile', authMiddleware, async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE users SET bio = $1, phone = $2, name = $3 WHERE id = $4',
      [bio ?? null, phone ?? null, name ?? null, userId]
    )
...
app.get('/api/reviews/:restaurantId', async (req, res) => {
  ...
  try {
    ...
    const total = await pool.query(`SELECT COUNT(*) FROM reviews WHERE restaurant_id=$1`, [restaurantId])
  ...
  try {
    const r = await pool.query(
      `SELECT rl.review_id FROM review_likes rl JOIN reviews rv ON rv.id=rl.review_id
       WHERE rl.user_id=$1 AND rv.restaurant_id=$2`,
...
app.get('/api/lists/:listId', optionalAuth, async (req, res) => {
  ...
  try {
    ...
    const lst = r.rows[0]
    if (!lst.is_public && (!req.user || req.user.id !== lst.user_id)) {
    ...
    res.json({ ...lst, items: items.rows.map(i => ({ ...i, images: i.image_url ? [i.image_url] : [] })),
               is_owner: req.user && req.user.id === lst.user_id })
  } catch (err) { res.status(500).json({ message: err.message }) }
})
...
app.get('/api/users/search', authMiddleware, async (req, res) => {
  ...
  try {
    const r = await pool.query(
      `SELECT u.id, u.username, f.id AS friendship_id, f.status AS friendship_status, f.requester_id
       ...
       WHERE LOWER(u.username) LIKE $2 AND u.id!=$1 AND u.is_verified=true AND u.username IS NOT NULL
    ...
    const users = r.rows.map(u => {
      ...
      return { id: u.id, username: u.username, friendship_id: u.friendship_id, relation: rel }
    })
    res.json({ users })
  } catch (err) { res.status(500).json({ message: err.message }) }
})
...
app.get('/api/friends/requests', authMiddleware, async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT f.id AS friendship_id, f.created_at, u.id AS requester_id, u.username AS requester_username
...
app.post('/api/friends/request/:addresseeId', authMiddleware, async (req, res) => {
  ...
  try {
    const target = await pool.query(`SELECT id FROM users WHERE id=$1 AND is_verified=true`, [addresseeId])
