      ...
      <div class="flex items-start justify-between gap-6 mb-3">
        <div class="min-w-0">
          <a class="font-headline italic text-3xl hover:text-primary transition-colors truncate block" href="${escapeHtml(
            href
          )}">${escapeHtml(r?.restaurant_name || slug || 'Restaurant')}</a>
    ...
    <div class="w-full max-w-xl bg-surface-container-lowest rounded-xl editorial-shadow p-8">
      ...
      <form id="editProfileForm" class="space-y-5">
        <label class="block">
          ...
          <input class="w-full bg-surface-container-lowest border-none rounded-full py-3 px-5 text-sm font-label focus:ring-1 focus:ring-primary-container outline-none editorial-shadow" id="editName" placeholder="Your name" value="${escapeHtml(user?.name || '')}" />
        </label>
        <label class="block">
...
async function init() {
  ...
  if (heroBio) {
    ...
    heroBio.textContent = bio || (isOwn ? 'No bio yet. Add one so friends know what you crave.' : '');
