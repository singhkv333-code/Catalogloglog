...
async function hydrateFriendActivity({ token }) {
  ...
  const enriched = await Promise.all(
    activity.map(async (item) => {
      ...
      const details = rid ? await loadRestaurantDetails(rid) : null;
      return { item, details };
    })
  );
  ...
  enriched.forEach(({ item, details }, idx) => {
    const friendName = item?.friend_username || 'Friend';
    const restaurantName = details?.name || item?.restaurant_name || item?.restaurant_id || 'Restaurant';
    const restaurantMeta = formatCuisineArea(details || item);
    ...
    const imgUrl = safeImgSrc(details?.image_url) || safeImgSrc(details?.images?.[0]);
    const avatarInitial = (friendName || 'F')[0]?.toUpperCase?.() || 'F';

    const slug = item?.restaurant_id || details?.slug || '';
    const href = slug
    ...
    if (header) {
      ...
      if (!avatarChip) {
        avatarChip = document.createElement('div');
        avatarChip.setAttribute('data-avatar-chip', '1');
        avatarChip.className =
          'w-12 h-12 rounded-full bg-surface-container-highest text-on-surface flex items-center justify-center font-label text-sm font-bold';
