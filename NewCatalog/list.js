...
function renderItemCard(item, { isOwner } = {}) {
  const slug = item?.restaurant_id || item?.slug || '';
  const name = item?.name || slug || 'Restaurant';
...
function openAddRestaurantModal({ token, listId, onAdded }) {
      ...
      try {
        ...
        results.innerHTML = items
          .map((r) => {
            ...
            return `
              <button type="button" class="w-full text-left flex items-center gap-4 p-4 rounded-xl bg-surface-container-low hover:bg-surface-container-highest transition-colors" data-action="add" data-slug="${escapeHtml(
                slug
              )}">
  ...
  results.addEventListener('click', async (e) => {
    const btn = e.target.closest('button[data-action="add"][data-slug]');
    if (!btn) return;
