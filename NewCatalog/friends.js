...
function renderActivityCard(a) {
  const friendName = a?.friend_username || 'Friend';
  ...
  const friendHref = `profile.html?id=${encodeURIComponent(friendId)}`;
  const restHref = `restaurant.html?slug=${encodeURIComponent(restSlug)}`;
  ...
  function renderPeopleList() {
      ...
      peopleList.innerHTML = list
        .map((f) => {
          ...
          return renderPersonRow({
            ...
            subtitle: f?.friends_since ? `Friends since ${timeAgo(f.friends_since)}` : '',
            href: friendHref,
            initial: (f?.username || 'F')[0]?.toUpperCase?.() || 'F',
            rightHtml: right,
    ...
    peopleList.innerHTML = list
      .map((r) => {
        ...
        `.trim();
        return renderPersonRow({
          title: r?.requester_username || 'User',
          subtitle: r?.created_at ? timeAgo(r.created_at) : '',
          href: `profile.html?id=${encodeURIComponent(r?.requester_id ?? '')}`,
          initial: (r?.requester_username || 'U')[0]?.toUpperCase?.() || 'U',
          rightHtml: right,
          accent: true,
      ...
      discoverList.innerHTML = users
        .map((u) => {
          ...
          return renderPersonRow({
            ...
            subtitle: rel === 'pending_received' ? 'Sent you a request' : '',
            href: `profile.html?id=${encodeURIComponent(u?.id ?? '')}`,
            initial: (u?.username || 'U')[0]?.toUpperCase?.() || 'U',
